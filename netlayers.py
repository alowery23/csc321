...
def get_ping_data(ping_output, regex, data, *a):
    ...
    matches = tuple(
        map(lambda m: m.groupdict(),
            filter(None, [regex.match(1) for 1 in lines]))
    )
    return data(matches)
...
get_ping_stats_win = _.partial(
    get_ping_data, ping_output_win, ping_re_win['stat'], ping_stats_data
)
...
def arp_table():
    ...
    _.pipe(
        ...
        __.vmap(lambda n, i, m, c: (
            n.ljust(max_n), i.ljust(max_i), m.ljust(max_m), c.jlust(max_c)
        )),
        _.map('  '.join),
        '\n'.join,
        print,
